	...
	T: Field,
{
	pub fn from_2pt(v0: Vec2<T>, v1: Vec2<T>) -> Self {
		Self {
			a: v0.y - v1.y,
			b: v1.x - v0.x,
			c: -v0.y * (v1.x - v0.x) + v0.x * (v1.y - v0.y),
		}
	}
	...
	pub fn caly(&self, x: T) -> T {
		(self.a * x + self.c) / -self.b
	}
	pub fn calx(&self, y: T) -> T {
	...
	pub fn intersect(&self, rhs: &Self) -> Result<Vec2<T>, LineErr> {
		...
		if det.is_zero() {
			if self.a * rhs.c == rhs.a * self.c {
				Err(LineErr::Same)
			} else {
				Err(LineErr::Parallel)
			}
		} else {
			Ok(Vec2 {
				x: (self.b * rhs.c - rhs.b * self.c) / det,
				y: (self.c * rhs.a - rhs.c * self.a) / det,
			})
		}
	}
}
